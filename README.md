...
![image](https://user-images.githubusercontent.com/60325113/119272670-c88ac100-bc0f-11eb-9baa-06a21a635e9c.png)
...
Так как база данных с конентом не является особо нагруженной, а пользовательские запросы по бизнес-логике составляют малую часть от ежедневного трафика, таблицы нет смысла шардировать. Основная нагрузка придется на просмотр видео, это будет описано далее.
...
Так как распределение CDN-серверов происходит внутри одной страны, нужно обеспечить наиболее точный механизм роутинга запросов за видео.
  
Мы будем располагать автономной системой, при этом будем использовать anycast и анонсы сети сделаем из разных датацентров, распределенных по всей стране, чтобы иметь несколько разных маршрутов к нашей АС. Это обеспечит оптимальную балансировку запросов от пользователей, так как благодаря anycast клиенты будут направляться к ближайшим серверам.
  
Когда пользователь делает запрос, запрос от его роутера посылается к провайдеру, который владеет некоторой автономной системой. Маршрутизатор этой автономной системы знает маршрут к нашей автономной системе, так как мы ее анонсировали. С помощью BGP определяется кратчайший маршрут к нашей АС. Далее пользователь попадает на ближайший с точки зрения топологии сети CDN. 

CDN в каждой точке содержит файловые сервера (так как контент занимает значительный объем приходится конфигурировать несколько файловых серверов на одном CDN) и севрер маршрутизации с таблицами соответствия файловых серверов и контента (можно строить таблицу хэшированием).

Можно сконфинурировать сервера по двум типам - одни будут хранить в себе наиболее популярный контент - 5% от всего, другие будут хранить весь контент (уже сжатый и преобразованный).

Исходя из метрик сервиса аналитики можно определять с какой-то периодичностью (например, раз в сутки) наиболее популярный контент каждого региона. Например, cамые популярные видео распределять по экземпляру по всем равнозначным нодам региона, средней популярности в двух или трех (распределять механизмом round-robin, чтобы все сервера загружать распределенно), менее популярный оставить в одном экземпляре, контент, который практически перестал запрашиваться, убирать из хранилища. 
Таким образом трафик будет более равномерно распределен. При его увеличении в дальнейшем, кол-во узлов хранения нужно увеличивать, чтобы было достаточно пропускной способности. Все распределение - номера нод в соответствии с контентом - должно храниться в таблице маршрутизации, которая была описана выше. 

Основная таблица соответствия распределения контента по всем ДЦ хранится также актуальной копией во всех сервисах маршрутизации каждого CDN и при условии, если запрос пользователя уже пришел в узел, а там не оказалось нужного контента, система с помощью этой таблицы определяет следующий ближайший к пользователю ДЦ с контентом в наличии. 

CDN-сервера располагаются у ISP (провайдеры интернета) или в IXP(точки обмена трафиком).
...
Количество: 
  * крупные - 0,4 * 5264 Гбит/сек / (25 * 2) = 43 * 2 (по реплике на каждый) = 86
  * мелкие - 0,6 * 5264 Гбит/сек / (50 * 2) = 32 * 3 = 96
...
280,45 / (30 * 4) = примерно 3 сервера.
Дополнительно для хранения копий контента в репликах будем использовать еще 6 серверов.
Будем использовать каждую реплику при отдаче контента, чтобы распределять самый популярный контент 
и трафик пользоваетля между ними.
...
Максимальная конфигурация сервера - 90 слотов под жесткие диски. Возьем 90 SSD по 4 Тб:
5 609,16 / 360 = 16 файловых серверов. Для отказоустойчивости можно сделать по 1й реплике 
...
| Мелкие CDN с популярным контентом | 16 | 512 | 9 файловых серверов x (30 SSD по 4 Тб) | 96 | 50 % - Москва и Санкт-Петербург, 50 % - остальные крупные города России (карта указана выше) |
| Крупные CDN | 16 | 512 | 16 файловых серверов x (90 SSD по 4 Тб) | 86 | 50 % - Москва и Санкт-Петербург 50 % - остальные крупные города России (карта указана выше) |

#### Схема

![image](https://user-images.githubusercontent.com/60325113/119275614-c54b0180-bc1e-11eb-8a7a-2720c5326f09.png)
